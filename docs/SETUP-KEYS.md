...
| `RAPIDAPI_KEY` (same key) | Fantastic Jobs — *LinkedIn Job Search API* and *Active Jobs DB*, via RapidAPI | https://rapidapi.com → subscribe to both (free Basic plan) | 25 requests + 250 jobs/month **each** (verified 2026-09-06); a 400 still costs a request | discovery: `linkedin_jobs` (LinkedIn's index, agencies excluded server-side) and `active_jobs_db` (employer career sites on 54 ATS platforms), 1 request/day each by config | no LinkedIn-indexed or career-site postings; the other boards still run |
